/**
 * QRCode.js - Standard standalone QR Code generator for Scholaris
 * Zero external dependencies, pure JS canvas/SVG/DOM generation
 */
(function(window){
      ...
      this.dataList.push({
        ...
        write: function(buffer) {
          for (var i = 0; i < data.length; i++) {
            ...
            if (c < 128) buffer.put(c, 8);
            else {
              var utf8 = encodeURIComponent(data.charAt(i)).substr(1).split('%');
              for (var j = 0; j < utf8.length; j++) buffer.put(parseInt(utf8[j], 16), 8);
            }
          }
        }
      });
  ...
  QRCode.prototype.render = function() {
    ...
    var nTileW = nWidth / nCount;
    var nTileH = nHeight / nCount;
    ...
    for (var row = 0; row < nCount; row++) {
      for (var col = 0; col < nCount; col++) {
        if (this._oQRCode.isDark(row, col)) {
          ctx.fillRect(Math.round(col * nTileW), Math.round(row * nTileH), Math.ceil(nTileW), Math.ceil(nTileH));
        }
      }
    }
